...
def pivot_reservas_lote_2(ware_code):
    with connections['gimpromed_sql'].cursor() as cursor:
        cursor.execute(f"""
            SELECT 
                r.CODIGO_CLIENTE,
...
@login_required(login_url='login')
def reporte_andagoya_bpa_odld(request):
    ...
    for i in inv_df['product_id'].unique():
        ...
        # df_product['total_unidades'] = df_product
...
def reporte_andagoya_bpa(request):
    ...
    inv_df['subtotal_unidades'] = (
        inv_df['numero_cajas'] * inv_df['unidades_caja']
        + inv_df['unidades_sueltas']
        + inv_df['unidades_estanteria']
    )
    ...
    df_last_columns = [
        ...
        'ware_code', 'location', 'unidades_caja',
        'numero_cajas', 'unidades_sueltas',
        'unidades_estanteria', 'subtotal_unidades',
        'total_unidades', 'diferencia',
        'observaciones', 'user__username'
    ]
...
def reporte_cerezos_bpa_old(request):
    ...
    df_unido['subtotal_unidades'] = (
        df_unido['numero_cajas'].replace('', '0').astype('int') * 
        df_unido['unidades_caja'].replace('', '0').astype('int') + 
        df_unido['unidades_sueltas'].replace('', '0').astype('int')
    )
...
def reporte_cerezos_bpa(request):
    ...
    df_last = [
        ...
        'unidades_sueltas', 'total_unidades',
...
def resumen_total_unificado(request):
    ...
    incluyendo reservas de Andagoya y comparaciones con MBA.
    """
    ...
    # AGRUPAR inventario de Cerezos (sin reservas)
    ...
    resumen_total['tf_total'] = (
        ...
        resumen_total.get('egreso_temp', 0)
    )

...
from api_mba.tablas_warehouse import (
    ...
    odbc_actualizar_reservas_lotes_warehouse,
...
ssh_host = '10.10.3.4'
ssh_username = 'root'
ssh_password =  'Gimapp3297#/*' # 'Gimcen2025/*$' #'Gimcen2021'
...
def stock_lote(request):
    ...
    if request.method == "POST":
        ...
        if todo:
            ...
            if obtener_conexion_config('reservas_lote') == 'api':
                api_actualizar_reservas_lotes_warehouse()
            elif obtener_conexion_config('reservas_lote') == 'odbc':
                odbc_actualizar_reservas_lotes_warehouse()
...
def obtener_ventas_porcliente_warehouse(codigo_cliente, desde, hasta):
    ...
    try:
        query = """
            SELECT 
                ...
                vf.QUANTITY,
                vf.UNIT_PRICE,
                (vf.QUANTITY * vf.UNIT_PRICE) AS PRECIO_TOTAL,
                vf.CODIGO_FACTURA
            FROM venta_facturas vf
            INNER JOIN clientes c 
                ON c.CODIGO_CLIENTE = vf.CODIGO_CLIENTE
        ...
        with connections['gimpromed_sql'].cursor() as cursor:
            cursor.execute(query, [codigo_cliente, desde, hasta])
        ...
        print("Error en obtener_ventas_porcliente_warehouse:", e)
...
def pipeline_picking_estadisticas_batch(request, year: int, month: int):
    ...
    if request.user.is_superuser:
        print(f'Inicia mineria de datos año: {year} - mes: {month}')
        ...
        print(f'Fin mineria de datos año: {year} - mes: {month}')
        return HttpResponse('ok')
